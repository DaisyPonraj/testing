...
def login():
    def invalid_un():
        username = browser.find_element_by_xpath(username_x).send_keys(invalid_un)
        time.sleep(3)
        password = browser.find_element_by_xpath(password_x).send_keys(PWD)
    ...
    def valid_un_pwd():
        username = browser.find_element_by_xpath(username_x).send_keys(UN)
        time.sleep(3)
        password = browser.find_element_by_xpath(password_x).send_keys(PWD)
        login_into = browser.find_element_by_xpath(login_x).click()
